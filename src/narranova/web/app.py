...
class NarranovaWebApp:
    ...
    def _book(self, book_id: str, environ: dict[str, object], csrf: str) -> str:
        ...
        body = f"""<a class="back" href="/">← Workspace</a><section class="book-head"><div><p class="eyebrow">Narration plan · revision {record['revision']}</p><h1>{self._e(book.title)}</h1><p>{self._e(book.author or 'Unknown author')} · {len(plan.chapters)} sections · {enabled_units} of {len(plan.units)} units included</p></div><span class="status status-{self._e(book.status)}">{self._e(book.status)}</span></section>
    ...
    def _job(self, job_id: str, environ: dict[str, object], csrf: str) -> str:
        ...
        chunk_rows = "".join(
            f"""<article class="chunk-row"><div><span class="mono">{self._e(chunk.id)}</span><strong>{self._e(chunk.status)}</strong><small>{chunk.attempts} attempt{'s' if chunk.attempts != 1 else ''}{f' · {chunk.duration_seconds:.1f}s' if chunk.duration_seconds else ''}</small></div>{f'<audio controls preload="none" src="/jobs/{self._e(job_id)}/chunks/{self._e(chunk.database_id)}/audio"></audio>' if chunk.status == 'completed' else ''}</article>"""
            for chunk in chunks
        )
