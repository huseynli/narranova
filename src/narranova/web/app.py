...
class NarranovaWebApp:
    ...
    def _layout(self, title: str, body: str, environ: dict[str, object], refresh: bool = False) -> str:
        ...
        return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light dark"><title>{self._e(title)} · Narranova</title>{refresh_tag}<script>let savedTheme="";try{{savedTheme=localStorage.getItem("narranova-theme")||""}}catch(error){{}}const cookieTheme=document.cookie.split(";").map(value=>value.trim()).find(value=>value.startsWith("narranova_theme="))?.split("=")[1]||"";const initialTheme=[savedTheme,cookieTheme].find(value=>value==="light"||value==="dark")||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");document.documentElement.dataset.theme=initialTheme</script><link rel="stylesheet" href="/static/app.css"><link rel="stylesheet" href="/static/choices.css"><script defer src="/static/app.js"></script></head><body><header class="topbar"><div class="topbar-inner"><a class="brand" href="/"><span>N</span><strong>Narranova</strong></a><nav aria-label="Primary navigation">{nav_html}</nav><div class="topbar-actions"><button class="theme-toggle" type="button" data-theme-toggle aria-pressed="false"><span class="theme-icon" aria-hidden="true"></span><span data-theme-label>Dark</span></button></div></div></header><div class="shell">{notice_html}{body}</div><footer>Narranova · Local-first audiobook production</footer></body></html>"""
