...
class NarranovaWebApp:
    ...
    def _dashboard(self, environ: dict[str, object], csrf: str) -> str:
        ...
        <aside class="stack"><form class="import-card" method="post" action="/actions/import" enctype="multipart/form-data">{self._csrf(csrf)}<div class="import-card-copy"><p class="eyebrow">New book</p><h2>Import an EPUB</h2><p>Add a DRM-free book to your production library.</p></div><label for="epub">Book file</label><div class="import-actions"><input id="epub" name="epub" type="file" accept=".epub,application/epub+zip" required><button class="primary">Import book</button></div></form><section class="panel" id="recent-jobs"><header><div><p class="eyebrow">Activity</p><h2>Recent jobs</h2></div><span class="count">{len(all_jobs):02d}</span></header>{job_rows}</section></aside></div>"""
